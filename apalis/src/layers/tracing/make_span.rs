...
/// [`Span`]: tracing::Span
/// [`Trace`]: super::Trace
pub trait MakeSpan<Args, Id> {
    /// Make a span from a request.
    fn make_span(&mut self, request: &Task<Args, Id>) -> Span;
}

impl<Args, Id> MakeSpan<Args, Id> for Span {
    fn make_span(&mut self, _request: &Task<Args, Id>) -> Span {
        self.clone()
    }
}

impl<F, Args, Id> MakeSpan<Args, Id> for F
where
    F: FnMut(&Task<Args, Id>) -> Span,
{
    fn make_span(&mut self, request: &Task<Args, Id>) -> Span {
        self(request)
    }
...
impl<Args, Id: Display> MakeSpan<Args, Id> for DefaultMakeSpan {
