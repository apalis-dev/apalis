...
pub trait OnRequest<Args, Id> {
    ...
    fn on_request(&mut self, request: &Task<Args, Id>, span: &Span);
}

impl<Args, F, Id> OnRequest<Args, Id> for F
where
    F: for<'a> FnMut(&'a Task<Args, Id>, &'a Span),
{
    fn on_request(&mut self, request: &Task<Args, Id>, span: &Span) {
        self(request, span)
    }
...
impl<Args, Id> OnRequest<Args, Id> for DefaultOnRequest {
    fn on_request(&mut self, _: &Task<Args, Id>, _: &Span) {
        match self.level {
            Level::ERROR => {
